...
public:
    ...
    /// Attempt to find a subrect of a surface, otherwise loads a texture from 3DS memory to OpenGL and caches it (if not already cached)
    CachedSurface* GetSurfaceRect(const CachedSurface& params, bool match_tiling, bool match_res_scale, bool load_if_create, MathUtil::Rectangle<int>& out_rect);
