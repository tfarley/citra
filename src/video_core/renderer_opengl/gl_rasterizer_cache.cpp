...
CachedSurface* RasterizerCacheOpenGL::GetSurface(const CachedSurface& params, bool match_tiling, bool match_res_scale, bool load_if_create) {
    using PixelFormat = CachedSurface::PixelFormat;
    using SurfaceType = CachedSurface::SurfaceType;
...
CachedSurface* RasterizerCacheOpenGL::GetSurfaceRect(const CachedSurface& params, bool match_tiling, bool match_res_scale, bool load_if_create, MathUtil::Rectangle<int>& out_rect) {
    if (params.addr == 0) {
        return nullptr;
    }
    ...
    for (auto it = surface_cache.lower_bound(surface_interval); it != cache_upper_bound; ++it) {
        for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
            ...
            if (params.addr >= surface->addr &&
                ...
                CachedSurface::CheckFormatsBlittable(params.pixel_format, surface->pixel_format) &&
                CachedSurface::GetFormatBpp(params.pixel_format) == CachedSurface::GetFormatBpp(surface->pixel_format))
            {
                // Make sure optional param-matching criteria are fulfilled
                bool tiling_match = (params.is_tiled == surface->is_tiled);
                bool res_scale_match = (params.res_scale_width == surface->res_scale_width && params.res_scale_height == surface->res_scale_height);
    ...
    return GetSurface(params, match_tiling, match_res_scale, load_if_create);
}
...
std::tuple<CachedSurface*, CachedSurface*> RasterizerCacheOpenGL::GetFramebufferSurfaces(const Pica::Regs::FramebufferConfig& config, MathUtil::Rectangle<int>& rect) {
    ...
    CachedSurface* color_surface = using_color_fb ? GetSurfaceRect(color_params, true, true, true, color_rect) : nullptr;

    MathUtil::Rectangle<int> depth_rect;
    CachedSurface* depth_surface = using_depth_fb ? GetSurfaceRect(depth_params, true, true, true, depth_rect) : nullptr;
    ...
    if (color_surface != nullptr && depth_surface != nullptr && (depth_rect.left != color_rect.left || depth_rect.top != color_rect.top)) {
        ...
        if (depth_rect.left != 0 || depth_rect.top != 0) {
            depth_surface = GetSurface(depth_params, true, true, true);
        }

        rect = MathUtil::Rectangle<int>(0, 0, color_params.width, color_params.height);
